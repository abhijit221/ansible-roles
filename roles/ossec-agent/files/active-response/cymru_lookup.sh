...
SERVICE=$7
FILENAME=$8
# The OSSEC server passes the filename as $7 - test for that
FILENAME=${8:-$SERVICE}
...
# Alert or exit
MSG1="${FILENAME:-(empty)} ($SHA1) on ${AGENT:-(empty)}"
[[ "$RESULT" =~ '127.0.0.' ]] && log "WARNING: Malicious hash found for $MSG1"
send_cdb "$SHA1" "$FILENAME"
die "OK: No match found for $MSG1"
